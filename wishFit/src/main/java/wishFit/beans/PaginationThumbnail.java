...
public class PaginationThumbnail {
	...
	public void calculate() throws Exception{
		...
		if(isSearch()) { // 완전검색이라면
			this.count = imageDao.count(boardLargeName, boardMiddleName, column, keyword);
		}
		else if(likeList()) {
			this.count = imageDao.likeCount(boardLargeName, boardMiddleName);
		}
		else if(isMiddle()) { // 중분류
			this.count = imageDao.middleCount(boardLargeName, boardMiddleName);
		}
		else if(largeSearch()) { // 대분류에서 검색한거면
			this.count = imageDao.largeSearchCount(boardLargeName, column, keyword);
		}
		else if(isLarge()) { // 대분류만
			this.count = imageDao.largeCount(boardLargeName);
		}
		else if(totalSearch()){ // 전체글에서의 검색이면
			this.count = imageDao.count(column, keyword);
		}
		else { // 전체글
			this.count = imageDao.count();
		}
		...
		if(this.isSearch()) { // 완전검색시
			this.list = imageDao.searchList(boardLargeName, boardMiddleName, column, keyword, begin, end);			
		}
		else if(this.likeList()) { // 추천순

			this.list = imageDao.likeList(boardLargeName, begin, end);
		}
		else if(this.isMiddle()) { // 중분류
			this.list = imageDao.middleList(boardLargeName, boardMiddleName, begin, end);
		}
		else if(this.largeSearch()) { // 대분류+검색
			this.list = imageDao.largeSearchList(boardLargeName, column, keyword, begin, end);
		}
		else if(this.isLarge()){ // 대분류만
			this.list = imageDao.largeList(boardLargeName, begin, end);
		}
		else if(totalSearch()) {
			this.list = imageDao.totalSearchList(column, keyword, begin, end);
		}
		else {
			this.list = imageDao.listByRownum(begin, end);
		}
