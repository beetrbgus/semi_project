...
public class Noti_Pagenation {
	...
	public Noti_Pagenation(HttpServletRequest req) {
		try {
	...
	public void calculate() throws Exception {
		...
		if(isSearch()) {
			this.count = notificationDao.count(uid , column, keyword);
		}
		else {
			this.count = notificationDao.count(uid);
		}
		...
		//list 계산
		if(this.isSearch()) {
			this.list = notificationDao.searchByRownum(column, keyword, begin, end);
		}
